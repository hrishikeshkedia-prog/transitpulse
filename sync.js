...
// Loaded synchronously after the main app script (before DOMContentLoaded fires).
(function () {
  'use strict';
  ...
  function pullAll() {
    ...
    api('GET', '/api/data').then(function (d) {
      ...
      var map = { tr:'trips', py:'payments', iv:'invoices',
                  cl:'clients', co:'COMPANY', fi:'finData', ml:'savedMails' };
  ...
  // ── Patch doLogout: clear persisted session and JWT ───────────────────────────
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // LOGIN SCREEN  (injected into the onboarding card)
  // ═══════════════════════════════════════════════════════════════════════════════
  function injectLoginScreen() {
    ...
    tReg.id = 'tab-reg';
    tReg.textContent = 'New Account';
    _tabStyle(tReg, true);
    tReg.onclick = function () { showTab('register'); };

    var tLog = document.createElement('button');
    tLog.id = 'tab-log';
    tLog.textContent = 'Sign In';
    _tabStyle(tLog, false);
    tLog.onclick = function () { showTab('login'); };

    tabBar.appendChild(tReg);
    tabBar.appendChild(tLog);
    ...
    loginDiv.innerHTML =
      '<div style="padding:24px 24px 20px">' +
        ...
        '<div class="ob-f"><label>Username</label>' +
          '<input type="text" id="li_user" placeholder="Your username" autocomplete="username" ' +
          'style="width:100%" onkeydown="if(event.key===\'Enter\')document.getElementById(\'li_pass\').focus()"></div>' +
        '<div class="ob-f"><label>Password</label>' +
          '<input type="password" id="li_pass" placeholder="Your password" autocomplete="current-password" ' +
          'style="width:100%" onkeydown="if(event.key===\'Enter\')fdpLogin()"></div>' +
        '<div id="li_err" style="font-size:12px;color:#dc2626;min-height:18px;margin:6px 0"></div>' +
        '<button onclick="fdpLogin()" ' +
  ...
  function _tabStyle(btn, active) {
    btn.style.cssText =
      'flex:1;padding:13px;border:none;cursor:pointer;font-size:13px;font-weight:600;' +
      'font-family:Inter,sans-serif;transition:all .15s;' +
      (active
        ? 'background:#fff;color:#2563EB;border-bottom:2px solid #2563EB;'
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // SERVER SYNC SETTINGS CARD  (injected into Settings page)
  // ═══════════════════════════════════════════════════════════════════════════════
  function injectServerSettings() {
    ...
    card.innerHTML =
      ...
      '</div>' +
      '<div class="cb">' +
        '<div class="ib" style="margin-bottom:14px">Connect to a self-hosted FreightDesk Pro server ' +
          'for multi-device sync and multi-user access. Leave blank for <strong>local-only</strong> mode.</div>' +
        '<div class="fgr">' +
  ...
  window.srvTest = function () {
    ...
    fetch(url + '/api/health')
      ...
      .then(function (d) {
        ...
        msg.textContent = '✓ Server online — FreightDesk Pro v' + (d.version || '?');
      })
      .catch(function () {
  ...
  window.srvConnect = function () {
    ...
    fetch(clean + '/api/health')
      .then(function (r) { return r.json(); })
      .then(function () {
        setBase(clean);
        msg.style.color = '#059669';
        msg.textContent = '✓ Connected. Sign in below (or re-login from the app) to start syncing.';
        refreshSrvUI();
      })
      .catch(function () {
        msg.style.color = '#dc2626';
  ...
  window.srvLogin = function () {
    ...
    api('POST', '/api/auth/login', { username: u, password: p })
      .then(function (data) {
        setToken(data.token);
        msg.style.color = '#059669';
        msg.textContent = '✓ Signed in as ' + (data.name || data.username) + '. Syncing…';
        refreshSrvUI(); pullAll();
      })
  ...
  window.srvDisconnect = function () {
    setBase(''); setToken('');
    var msg = document.getElementById('srvMsg');
    if (msg) msg.textContent = 'Disconnected. Running in local-only mode.';
    refreshSrvUI();
  };

  // ── Re-login overlay (JWT expired mid-session) ────────────────────────────────
  window.doSrvLogin = function () {
    ...
    api('POST', '/api/auth/login', { username: u, password: p })
      .then(function (data) {
        setToken(data.token);
        document.getElementById('srvLoginOverlay').remove();
        pullAll(); setSyncStatus('synced');
      })
      .catch(function (e) { err.textContent = e.message; });
  };

  window.dismissSrvLogin = function () {
    var el = document.getElementById('srvLoginOverlay');
    if (el) el.remove();
  };
  ...
  function showServerReloginOverlay(username) {
    ...
    ov.innerHTML =
      '<div style="background:#fff;border-radius:16px;padding:28px;width:340px;max-width:calc(100vw - 32px);box-shadow:0 20px 60px rgba(0,0,0,.3)">' +
        ...
        '<div style="margin-bottom:10px"><label style="font-size:11px;font-weight:600;color:#555;text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:4px">Username</label>' +
          '<input id="srl_u" type="text" value="' + (username || '') + '" readonly ' +
          'style="width:100%;padding:10px;border:1.5px solid #ddd;border-radius:6px;font-size:14px;background:#f5f5f5"></div>' +
        '<div style="margin-bottom:10px"><label style="font-size:11px;font-weight:600;color:#555;text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:4px">Password</label>' +
          '<input id="srl_p" type="password" placeholder="Your password" ' +
          'style="width:100%;padding:10px;border:1.5px solid #ddd;border-radius:6px;font-size:14px" ' +
          'onkeydown="if(event.key===\'Enter\')doSrvLogin()"></div>' +
  ...
  function setSyncStatus(state) {
    var map = {
      syncing: { text: '☁ Syncing…', color: '#2563EB' },
      synced:  { text: '☁ Synced',   color: '#059669' },
      error:   { text: '☁ Error',    color: '#dc2626' },
      none:    { text: '⊙ Local',    color: '#888'    }
    };
    var s = map[state] || map.none;
    ['syncStatusBadge', 'sbSyncBadge'].forEach(function (id) {
  ...
  function injectSidebarSyncBadge() {
    ...
    badge.id = 'sbSyncBadge';
    badge.style.cssText = 'font-size:10px;font-weight:600;margin-bottom:8px;letter-spacing:.03em;cursor:pointer';
